...
class CfnTemplateGenerator:
    ...
    def generate(self, request: DeployRequest) -> CfnBundle:
        ...
        readme = self._generate_readme(
            deployment_name, template_id, config, has_gateway, has_memory, has_policy, has_mcp_server
        )
    ...
    def _init_template(self, deployment_name: str, template_id: str | None) -> dict:
        return {
...
'''
            ...
            "Properties": {
                ...
                "KnowledgeBaseConfiguration": {
                    ...
                    "VectorKnowledgeBaseConfiguration": {
                        "EmbeddingModelArn": {
                            "Fn::Sub": f"arn:aws:bedrock:${{AWS::Region}}::foundation-model/{embedding_model_id}"
                        },
                    },
                },
    ...
    def _add_policies(self, template: dict, policy_config: dict | None) -> None:
        ...
        for i, pol in enumerate(policies):
            ...
            if not statement:
                statement = (
                    "permit(principal, action, resource is AgentCore::Gateway) when { context.authenticated == true };"
                )
    ...
    def _generate_deploy_script(
        self, deployment_name: str, config: RuntimeConfig, has_mcp_server: bool, bundle_key: str = STRANDS_BUNDLE_KEY
    ) -> str:
        mcp_upload = ""
        if has_mcp_server:
            mcp_upload = """
