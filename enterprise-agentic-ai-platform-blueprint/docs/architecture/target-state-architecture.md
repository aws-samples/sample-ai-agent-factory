...
depends on a _cross-account_ boundary between environments — that a non-production principal cannot
...
profile can prove _within-environment_ boundaries (SCP denials, IAM trust, tenant isolation,
deployment boundary) against live services; it cannot prove _between-environment_ isolation, because
...
2. **Deploy.** Stable Gateway, validator, and GatewayAdmin roles are created in a prerequisite
   ...
   principal. Lambda binds each grant to the role's RoleId, so the permission phase is retired
   before a granted role is deleted and re-enabled once it exists again; a stale grant blocks every
   later grant on that alias. No Platform-side tenant/agent configuration is used to infer a
   principal. SCP-09
...
violations. What changes is the _endpoint_ `LiteLLMModel` points at: an AgentCore Gateway inference
...
| **U-2** | Native rate limiting has the §7.2 dimensions, evaluates before Policy, and fails open                                               | **Verified as traffic shaping only** (live 2026-09-24, pipeline-owned nonproduction Gateway, `evidence/live/2026-09-24-load-rate-limit.md`): the zero-rate wildcard returns exact HTTP 429; the per-model RPM/TPM entries are approximate — 14 back-to-back requests in 8 s all passed, paced traffic admitted 15–23 requests per minute with scattered 429s, and 22k–41k input tokens per minute were admitted against 10,000 TPM (budget-based, catches up late per the service documentation). Bedrock Mantle `Inferences`/`TotalInputTokens` corroborate. Rate limits are therefore never the quota or abuse boundary. **Open:** Policy independence with customer limits absent, fail-open injection, missing-telemetry alarming | The extended enforcement and observability claims for §7, and cases A-10, A-11                                                         |
...
| **D-D** | Whether Cedar evaluation moves from inside each tool Lambda to the policy engine now, or stays until pipeline parity is proven      | **Open live migration.** The current wrapper is Verified, the isolated Gateway PolicyEngine contract passed in `us-west-2`, and an opt-in Workload pipeline path now supports `LOG_ONLY`/`ENFORCE`; reviewed deployment, parity, rollback, teardown, and wrapper retirement remain open                                                                                                                                                                                                                                                                                                                                                                                                                                               | §6 policy-engine claims, case A-5                                                                                                      |
