...
| RuntimeMemory `CreateWorkloadIdentity` reports `already exists` as `ValidationException`; `TagResource` on the retained identity returns `InternalServerErrorException` | A prior failed custom-resource create left the exact deterministic identity without ownership tags, and AgentCore does not support tagging an existing WorkloadIdentity in place (deterministic service 500, reproduced with the CLI)                                                   | Verify exact name and ARN; for the zero-tag shape only, delete the retained identity and recreate it with create-time tags, then re-read all five ownership tags; refuse (never adopt, never delete) any foreign or partial tag set                                                                                                                                                            |
| RuntimeMemory identity custom resource denies `TagResource`, then `ListTagsForResource`, on `workload-identity-directory/default`                                       | Per the `bedrock-agentcore` service reference, every WorkloadIdentity / Oauth2CredentialProvider action authorizes on both the named resource and its parent container (`workload-identity-directory`, `token-vault`); exact-ARN or family-only scoping fails closed one call at a time | Grant the handler's complete action set (create/get/update/delete, `TagResource`, `ListTagsForResource`) on the two default containers plus the two deterministic families -- never a bare `*` -- and prove it with `iam:SimulateCustomPolicy` for every (action, container/named resource) pair before redeploying (`scripts/live-agentcore-generated-agent-spike/simulate_idprov_policy.py`) |
