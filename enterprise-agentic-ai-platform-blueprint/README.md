...
1. **Synth** — the target SSOT is the GA AWS Agent Registry (`packages/agent-registry/`). Revision R1 adds native `AWS::AgentRegistry::Registry` and `RegistryRecord` resources, custom governance documents, a conditioned `RegistryReaderRole`, and versioned SSM discovery parameters alongside the unchanged DynamoDB rollback path. The Workstream consumer switch is deliberately deferred to R2 and remains unverified until the Platform producer has passed pipeline deployment, approval, read, rollback, and teardown gates.
...
- **Pipeline-owned GA Agent Registry migration** — the isolated native-resource and approval contract passed, but the blue-green Platform deployment, Workstream consumer switch, cross-account reader path, rollback, and teardown are not yet live-verified.
