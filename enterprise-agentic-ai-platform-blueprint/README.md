...
> **Accepted target state — implementation in progress.** The revamp converges D-01 and D-03 into one topology: one consolidated Management/Governance account, environment-isolated Platform accounts, and per-workstream accounts. The golden-path inference boundary is Amazon Bedrock AgentCore Gateway with a Bedrock Mantle inference target; generated agents use `LiteLLMModel` against its OpenAI-compatible endpoint. The compatibility spike is live-proven in `us-west-2`: Cognito M2M, model discovery, streaming and non-streaming inference, exact HTTP 429 rate limiting, and zero-residue cleanup all passed. On 2026-09-19 the native CloudFormation resources completed real Platform pipeline executions through Source, Synth, SelfMutate, nonproduction deployment, manual approval, and production deployment. The pipeline-created production Gateway and target are `READY`, its rate limit is `ACTIVE`, its baseline Guardrail is `READY`, and the Management/Governance Log Archive is live. Cognito M2M, 49-model discovery, and Strands `LiteLLMModel` streaming and non-streaming inference also passed against that pipeline-owned endpoint. An isolated `us-west-2` PolicyEngine run on exact commit `46c3a62` passed eight strict policies, a 20-case `sub`/group matrix, filtered `tools/list`, direct-call denial, exact JWT rejection statuses, mode rollback, and independent zero-residual inventory. An isolated GA Agent Registry run on exact commit `8e66dc3` proved native CloudFormation resources, a custom governance-document round trip, explicit `DRAFT → submit → APPROVED`, data-plane discovery, deterministic update rollback, and independent zero-residual inventory. On 2026-09-20 pipeline-owned R1 producer commit `f3ec7d6` deployed through both Platform environments; template-bound utility commit `39a13ab` observed and explicitly approved both records per environment, independent read-back matched descriptor digests and exact discovery sets, and wrong-principal/wrong-account trust twins were denied. Pipeline-owned Workstream Runtime/Memory/Gateway/Policy integration, the R2 Registry consumer, full adversarial tests, rollback, and final teardown remain mandatory release gates; OTEL span correlation remains a reproduced blocker. See [`docs/architecture/target-state-architecture.md`](docs/architecture/target-state-architecture.md), [ADR-0016](docs/adr/ADR-0016-agentcore-gateway-inference-supersedes-litellm-proxy.md), [`evidence/live/2026-09-18-agentcore-gateway-spike.md`](evidence/live/2026-09-18-agentcore-gateway-spike.md), [`evidence/live/2026-09-19-platform-pipeline-deployment.md`](evidence/live/2026-09-19-platform-pipeline-deployment.md), [`evidence/live/2026-09-19-policyengine-compatibility-spike.md`](evidence/live/2026-09-19-policyengine-compatibility-spike.md), [`evidence/live/2026-09-19-agent-registry-compatibility-spike.md`](evidence/live/2026-09-19-agent-registry-compatibility-spike.md), and [`evidence/live/2026-09-20-pipeline-ga-agent-registry-r1.md`](evidence/live/2026-09-20-pipeline-ga-agent-registry-r1.md).
...
- **Pipeline-owned GA Agent Registry R2 consumer migration** — R1 producer deployment and approval passed in both Platform environments, but the Workstream consumer switch, matching-validator positive cross-account read, wrong-ExternalId/session twins, consumer rollback, and teardown are not yet live-verified.
...
The teardown refuses to synthesize a present Platform Gateway or pipeline stack without the model-rate allocation and its account/role context. For an R2 Workload deployment, also provide both resolved GA context files, stable tool IDs, Workstream account/AZ tuples, and the Gateway Region; the script destroys production/nonproduction ToolGateway stacks before their RegistryRoles stacks and then removes the Workload pipeline root. Before each stack deletion, the script snapshots the exact physical IDs of its CodeBuild projects and Lambda functions; after a successful `cdk destroy`, it deletes only those exact service-created default CloudWatch log groups. Missing groups are idempotent, while discovery or deletion errors fail the teardown. A direct `cdk destroy` bypasses this cleanup and can leave empty `/aws/codebuild/*` or `/aws/lambda/*` groups. `cdk destroy` runs dependency-ordered. The EU AI Act Object-Lock COMPLIANCE 7-year bucket cannot be deleted before its retention expires — this is intentional and documented.
