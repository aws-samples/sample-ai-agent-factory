...
| RuntimeMemory `CreateWorkloadIdentity` denies `bedrock-agentcore:TagResource` on `.../workload-identity/*`                | Tags passed at create time are authorized as an implicit `TagResource` against the resource FAMILY ARN, because the named resource does not exist yet; exact-ARN scoping alone fails closed | Grant create-time `TagResource` on exactly the two deterministic family ARNs (`token-vault/default/oauth2credentialprovider/*`, `workload-identity-directory/default/workload-identity/*`); keep post-create `ListTagsForResource`/`TagResource` on the exact ARNs; never a bare `*` |
