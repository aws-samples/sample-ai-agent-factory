...
- **Platform pipeline deployment** (`us-west-2`, 2026-09-19): exact commit `f037b4e` completed Source, Synth, SelfMutate, assets, nonproduction deployment, manual approval, and production deployment. The Management/Governance Log Archive is live; the production Guardrail, AgentCore Gateway, and inference target are `READY`; and the native rate-limit resource is `CREATE_COMPLETE`. See [`evidence/live/2026-09-19-platform-pipeline-deployment.md`](evidence/live/2026-09-19-platform-pipeline-deployment.md).
...
- **Pipeline-owned production Gateway invocation** — deployment and readiness are live-verified, but Cognito M2M model discovery/inference and exact rate-limit behavior have not yet been rerun against this pipeline-created endpoint. Those behaviors are proven only by the isolated compatibility spike.
...
- **Regions** — `us-east-1` remains the live-verified D-03 tool-Gateway region; `us-west-2` is live-verified for both the central inference Gateway compatibility slice and the Platform pipeline deployment, but remains blocked for OTEL span correlation. APAC remains outside `PLATFORM_APPROVED_REGIONS`.
