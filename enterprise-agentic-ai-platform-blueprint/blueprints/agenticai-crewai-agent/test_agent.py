"""Unit tests for the CrewAI blueprint.
...
from crewai_agent import (  # type: ignore[import-not-found]
    CrewAIAgent,
    CrewAIAgentConfig,
...
def _cfg(**overrides) -> CrewAIAgentConfig:
    return CrewAIAgentConfig(
        tenant_id="demo",
        agent_id="primary",
        env_name="prod",
...
def test_invoke_known_tool_passes_protocol_version():
    gw = _FakeGateway()
    agent = CrewAIAgent(_cfg(), gw)
    res = agent.invoke_tool("target-demo___tool-echo", {"message": "hi"})
    assert res == {"ok": True}
    assert gw.calls[0]["version"] == MCP_PROTOCOL_VERSION


def test_invoke_unknown_tool_rejected():
    agent = CrewAIAgent(_cfg(), _FakeGateway())
    with pytest.raises(PermissionError):
        agent.invoke_tool("target-demo___nope", {})


def test_protocol_version_locked():
