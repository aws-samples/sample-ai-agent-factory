"""Path bootstrap for the runtime+memory spike test modules.
...
latter because the runtime+memory spike reuses the shared helpers in
``gateway_spike`` (``SpikeError``, ``Evidence``, ``JsonStore``, ...). It makes no
AWS calls at import or collection time.
