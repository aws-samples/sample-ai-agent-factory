...
- The opt-in PolicyEngine path passes 26 catalogue/compiler tests, 33 Workstream Gateway tests including executable association retry plus mode/detach convergence, 46 Workload pipeline tests, and 15 fail-closed teardown tests. The rendered template pins `FAIL_ON_ANY_FINDINGS`, per-policy `EnforcementMode: ACTIVE`, exact Gateway and principal resources, CMK grant constraints, six-minute IAM propagation, default `OFF` parity, and target → mode → policy → detach deletion dependencies. This is local proof only; pipeline/live parity remains a release gate.
