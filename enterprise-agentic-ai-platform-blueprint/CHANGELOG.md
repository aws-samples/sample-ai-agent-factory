...
- Added `eu-west-1` to the governed Region SSOT as the sole current EMEA onboarding target (AWS Agent Registry is available in Europe only in Ireland). All eight CDK app stages now resolve their Region through one fail-closed helper: ambient `CDK_DEFAULT_REGION` wins over optional explicit context, and no stack silently falls back to `us-west-2` or `us-east-1`. The misleading repository-wide `agenticai/defaultRegion` and dead `agenticai/approvedRegions` defaults were removed. The shared AgentCore VPC AZ-ID map now drives both network implementations for `us-east-1`, `us-west-2`, and `eu-west-1` (`euw1-az1/az2/az3`); an unreviewed Region fails synthesis unless the caller supplies an explicit set. Five targeted suites (32 tests), mutation checks for Region inclusion/precedence/bin wiring/AZ IDs, build, lint, and strict `eu-west-1` management synth pass. A second strict synth of the actual three-account Workload pipeline set `AWS_REGION`, `AWS_DEFAULT_REGION`, and `CDK_DEFAULT_REGION` to Ireland (the CDK CLI otherwise derives the child app Region from its SDK session): every root and nested manifest targeted `eu-west-1`, the feature branch was explicit, and cdk-nag had zero unsuppressed findings. This is offline onboarding only; live EMEA readiness remains gated by the independent pipeline, positive/adversarial, rollback, observability, and teardown matrix.
- Added `pipelines/bootstrap/render-cfn-execution-policy.py`, a deterministic Platform/Workstream/Management policy generator derived from the live-proven policies. It emits Region- and account-scoped Registry, cross-account role, bootstrap-parameter, stream and Lambda ARNs; includes the live-proven `iam:ListEntitiesForPolicy` delete path; and preserves service-conditioned `iam:PassRole`. Real `eu-west-1` renderings for all three accounts passed IAM Access Analyzer with zero findings and nine exact allow/deny simulator twins. `bootstrap-cross-account.sh` now requires an explicit Region, resolves a shared policy name to each target account's local ARN, refuses multi-account reuse of one account-scoped ARN, and rejects `AdministratorAccess`. Eight CLI integration tests, 46 pipeline conformance tests, Ruff, and three mutation checks pass.
- Removed implicit `us-west-2`/`us-east-1` defaults from all live AgentCore probes, the mandatory evaluation/smoke gates, SCP soak, direct and pipeline teardown, stranded-Gateway recovery, and adversarial/integration/teardown AWS session fixtures. Destructive and attestation paths now require `--region`, `CDK_DEFAULT_REGION`, `AWS_REGION`, or `AWS_DEFAULT_REGION` as appropriate and fail before any AWS call when absent. A repository-wide source regression, 325 affected Python tests, 20 teardown conformance tests, and the recovery shell harness pass; restoring one probe default is mutation-caught.
- Replaced the Platform tool catalogue's hard-coded `us-east-1` Lambda targets with validated `${PLATFORM_REGION}` + `${PLATFORM_ACCOUNT_ID}` placeholders. The GA producer and legacy D03 Registry seed now resolve both from the stack, and an `eu-west-1` synth twin proves records contain concrete Ireland alias ARNs with no placeholder leakage.
- Generated agent 1.3.0 makes the mandatory pipeline evaluation gate real: the nonproduction Runtime stack creates a Workstream-local invoker role trusted only by one stable Platform CodeBuild role and scoped only to that Runtime; `RuntimeArn` flows through a CloudFormation output; CodeBuild assumes the invoker and calls `InvokeAgentRuntime`, never Bedrock directly. Authorized responses now include final reply text plus Strands-measured input/output tokens, inference latency and guardrail outcome while Memory/evidence retain fingerprints only. GA generated-agent mode now runs `EvaluationGate` before production approval (it previously skipped the gate); Gateway-only compatibility mode remains unchanged.
- Evaluation cost no longer defaults every model to Sonnet pricing. AWS Price List-derived standard Mantle rates for `gpt-oss-120b` are pinned as of 2026-09-26 for the three approved Regions, including Ireland at $0.00018/$0.00070 per 1K input/output tokens; unknown Region/model pairs fail synthesis and missing/nonpositive runtime price inputs fail before AWS access. The four legacy direct-Bedrock/profile components (offline evaluation infrastructure, online evaluation, ECS LiteLLM, and direct circuit breaker) now fail synthesis in EU Regions until independent residency/routing proof exists.
