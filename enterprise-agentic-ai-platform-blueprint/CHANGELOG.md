...
- Fail-closed pipeline Runtime image gate that binds the CDK asset tag to its exact digest, starts or observes an ECR scan for that digest, blocks `CRITICAL`/`HIGH` findings and every terminal or unknown scan status, and exposes only the scanned digest to `AWS::BedrockAgentCore::Runtime`. The gate is reviewed locally; Workload-pipeline live proof remains pending.
