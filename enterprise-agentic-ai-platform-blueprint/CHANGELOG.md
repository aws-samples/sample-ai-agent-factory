...
- Opt-in Workload pipeline PolicyEngine path with explicit `OFF`/`LOG_ONLY`/`ENFORCE` modes, exact IAM or OAuth principals, one strict native policy per subscribed tool, CMK encryption, six-minute Gateway-role propagation, convergent mode/detach waiters, and wrapper-preserving rollback.
...
- `TODO-GW-POLICY-ENGINE` now reflects live migration debt rather than API availability: the Workload pipeline can opt into `LOG_ONLY` or `ENFORCE`, but the Lambda wrapper remains until behavior parity, mode rollback, and zero-residual teardown pass on the reviewed pipeline revision.
...
- The opt-in PolicyEngine path passes 26 catalogue/compiler tests, 32 Workstream Gateway tests including executable mode/detach convergence, 46 Workload pipeline tests, and 15 fail-closed teardown tests. The rendered template pins `FAIL_ON_ANY_FINDINGS`, per-policy `EnforcementMode: ACTIVE`, exact Gateway and principal resources, CMK grant constraints, six-minute IAM propagation, default `OFF` parity, and target → mode → policy → detach deletion dependencies. This is local proof only; pipeline/live parity remains a release gate.
