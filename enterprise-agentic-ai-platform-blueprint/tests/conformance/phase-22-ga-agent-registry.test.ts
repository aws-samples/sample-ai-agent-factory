...
describe("Phase 22 — GA Registry producer remains additive", () => {
  ...
  it("keeps each alias permission identity stable across a retire-and-regrant cycle", () => {
    // Live-proven 2026-09-24: Lambda stores a role principal as its RoleId and
    // rejects every later AddPermission on an alias whose policy still names a
    // deleted role ("The provided principal was invalid"), while CloudFormation
    // replaces resources create-before-delete. The supported repair is
    // therefore two-phase — permission phase off (statements removed), then on
    // again — so the permission identity must derive from the ARN alone: the
    // same ARN must yield the same logical id in both enabled synths, and the
    // disabled synth must carry no permission at all.
    const permissionIds = (template: Template): string[] =>
      Object.keys(template.findResources("AWS::Lambda::Permission")).sort();
    const granted = permissionIds(synth());
    expect(granted).toHaveLength(Object.keys(PLATFORM_TOOL_CATALOGUE).length);
    expect(permissionIds(synth("nonprod", false))).toHaveLength(0);
    expect(permissionIds(synth())).toEqual(granted);
  });
