...
export interface ScaffoldOptions {
  ...
  /** Deprecated in R2; environment Registry IDs are resolved by the pipeline. */
...
const REGISTRY_ID_REGEX =
  /^reg-[a-z0-9-]{1,64}$|^[a-z][a-z0-9-]{1,128}$|^[A-Za-z0-9]{12,32}$/;
...
export function scaffoldAgentRepo(opts: ScaffoldOptions): Map<string, string> {
  ...
  if (
    opts.platformRegistryId !== undefined &&
    !REGISTRY_ID_REGEX.test(opts.platformRegistryId)
  ) {
    throw new Error(
  ...
  if (
    opts.platformAccountId !== undefined &&
    !/^[0-9]{12}$/.test(opts.platformAccountId)
  ) {
    throw new Error(
  ...
  files.set(
    ...
    [
      ...
      "subscribe to a tool, run `agenticai registry subscribe <toolId>`",
      "and re-deploy.",
      '"""',
      "import os",
      "",
