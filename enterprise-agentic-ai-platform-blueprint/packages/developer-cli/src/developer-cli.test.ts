...
describe("scaffoldAgentRepo", () => {
  ...
  it("cdk.context.json contains tenantId, agentId, R2 mode and empty tool subscriptions", () => {
    const files = scaffoldAgentRepo({
      tenantId: "retail",
      agentId: "productqa",
      workstreamId: "retail",
    ...
    expect(ctx["agenticai/enableGaRegistryConsumer"]).toBe(true);
    expect(ctx["agenticai/gaRegistryExpectedToolIds"]).toEqual([]);
    expect(ctx["agenticai/d03RegistryId"]).toBeUndefined();
  });
...
describe("cdk-context helpers", () => {
  const baseCtx = readAgenticContext({
    ...
    "agenticai/gaRegistryExpectedToolIds": ["tool-echo"],
  });
  ...
  it("appendSubscription rejects malformed tool ids", () => {
    expect(() => appendSubscription(baseCtx, "NOT VALID")).toThrow(/toolId/);
  });
  ...
  it("validateForSynth returns specific missing-key errors", () => {
    ...
    expect(errs.some((e) => e.includes("gaRegistryExpectedToolIds"))).toBe(
      true,
    );
  });
...
describe("runLocalEval", () => {
  ...
  it("fails the cost category when one row blows the per-prompt USD ceiling", () => {
    const report = runLocalEval([
      makeRow(),
      makeRow({ caseId: "c-2", costUsd: 1.0 }),
    ]);
...
describe("renderPullRequestBody", () => {
  ...
  it("lists the subscribed tools and the eval table", () => {
    const body = renderPullRequestBody({
      tenantId: "retail",
      agentId: "productqa",
      workstreamId: "retail",
      subscribedToolIds: ["tool-echo", "tool-ping"],
