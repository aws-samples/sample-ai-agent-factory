...
::alert[**Agent Registry uses its own API namespace.** Registry calls below use `aws agent-registry-control` (not `aws bedrock-agentcore-control`) and the `agent-registry:` IAM prefix. AgentCore Runtime, Gateway, and Identity are unchanged. Requires **AWS CLI v2 ≥ 2.36.19**; the notebook path requires **boto3 ≥ 1.43.66**.]{type="warning"}
...
source /dev/stdin <<'BLOCK'
REGION=$(aws configure get region)
REGISTRY_ID=$(aws agent-registry-control list-registries --no-paginate --query "registries[0].registryId" --output text --region $REGION)
...
# Idempotent: a second run hits ConflictException because the record name and
# version are fixed. Treat "already exists" as success - the next block looks the
# record up by name anyway.
EXISTING_RECORD=$(aws agent-registry-control list-registry-records \
  --registry-id "$REGISTRY_ID" \
  --query "registryRecords[?name=='workshop_travel_agent'] | [0].recordId" \
  --output text --region $REGION 2>/dev/null)
if [ -n "$EXISTING_RECORD" ] && [ "$EXISTING_RECORD" != "None" ]; then
  echo "Record 'workshop_travel_agent' already registered ($EXISTING_RECORD) - skipping create"
else
  aws agent-registry-control create-registry-record --registry-id "$REGISTRY_ID" --name "workshop_travel_agent" --record-type AGENT --descriptors file:///tmp/descriptors.json --record-version "1.0" --region $REGION
fi
BLOCK
:::

You should see a response with the `recordArn` and `status`. `create-registry-record` does not return a `recordId` field — the ID is the last segment of the ARN, and the next block looks it up with `list-registry-records` anyway.
...
REGION=$(aws configure get region)
REGISTRY_ID=$(aws agent-registry-control list-registries --no-paginate --query "registries[0].registryId" --output text --region $REGION)
RECORD_ID=$(aws agent-registry-control list-registry-records --registry-id "$REGISTRY_ID" --query "registryRecords[?name=='workshop_travel_agent'] | [0].recordId" --output text --region $REGION)
...
for i in $(seq 1 12); do
  STATUS=$(aws agent-registry-control get-registry-record --registry-id "$REGISTRY_ID" --record-id "$RECORD_ID" --query "status" --output text --region $REGION)
...
if [ "$STATUS" = "DRAFT" ]; then
  aws agent-registry-control submit-registry-record-for-approval --registry-id "$REGISTRY_ID" --record-id "$RECORD_ID" --region $REGION
...
if [ "$STATUS" = "APPROVED" ]; then
  ...
  aws agent-registry-control update-registry-record-status --registry-id "$REGISTRY_ID" --record-id "$RECORD_ID" --status APPROVED --status-reason "Approved for workshop" --region $REGION
...
REGION=$(aws configure get region)
REGISTRY_ID=$(aws agent-registry-control list-registries --no-paginate --query "registries[?name=='workshop-registry'].registryId | [0]" --output text --region $REGION)
...
You should see the `workshop_travel_agent` record with type `AGENT` and status `APPROVED`.
